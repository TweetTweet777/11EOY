...
VALIDATION:
    FOR COLUMN% = 1 TO UBOUND(F.NAME)
        IF YEARS(COLUMN%) < 0 OR USAGE(COLUMN%) < 0 OR SEWAGE(COLUMN%) <> 1 OR SEWAGE(COLUMN%) <> 0 OR RUBBISH(COLUMN%) <> 1 OR RUBBISH(COLUMN%) <> 0 OR PRINT.OPTION$ <> "Y" OR PRINT.OPTION$ <> "N" THEN
            FLAG(COLUMN%) = 1
        ELSE
            FLAG(COLUMN%) = 0
        END IF
    ...
    FOR COLUMN% = 1 TO UBOUND(F.NAME)
        ...
        IF FLAG(COLUMN%) = 0 THEN
            ...
            IF SEWAGE(COLUMN%) + RUBBISH(COLUMN%) = 2 THEN
                TOTAL.CHG(COLUMN%) = TOTAL.CHG(COLUMN%) + SEWAGECHG% + RUBBISHCHG%
